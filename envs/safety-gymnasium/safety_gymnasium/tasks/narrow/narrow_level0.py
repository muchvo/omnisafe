...
class NarrowLevel0(BaseTask):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, config):
        ...
        agent_placements_width = self.palcement_cal_factor * 0.3
        agent_placements_lenth = self.palcement_cal_factor * 0.3
        center_x, center_y = self.palcement_cal_factor * 0, self.palcement_cal_factor * 0
        self.agent.placements = [
            (
                center_x - agent_placements_width / 2,
                center_y - agent_placements_lenth / 2,
                center_x + agent_placements_width / 2,
                center_y + agent_placements_lenth / 2,
            )
        ]
        ...
        self._add_geoms(Apples(**apple_config), Oranges(**orange_config))
    ...
    def calculate_reward(self):
        ...
        dist_apple = self.agent.dist_xy(self.apples.pos[0])
        dist_orange = self.agent.dist_xy(self.oranges_pos[0])
        ...
        for i in range(self.apples.num):
            ...
            if name in self.reached_apples:
                continue
            if self.agent.dist_xy(self.apples.pos[i]) <= self.apples.size:
        ...
        for i in range(self.oranges.num):
            ...
            if name in self.reached_oranges:
                continue
            if self.agent.dist_xy(self.oranges_pos[i]) <= self.oranges.size:
    ...
    def specific_reset(self):
        ...
        self.last_dist_apple = self.agent.dist_xy(self.apples.pos[0])  # pylint: disable=no-member
        self.last_dist_orange = self.agent.dist_xy(self.oranges_pos[0])
